...
def matrix_wx(i, j):
    ...
    for r in range(i, j+1):
        for l in range(i, r+1):
            for k in range(i, l+1):
                if score := (parameters["Gw"] + matrix_whx(i,r, k,l) + matrix_whx(k+1, j, l-1, r+1)) < best_score: best_score = score
    ...
    if score := (dangle_L(i, i+1, j-1) + dangle_R(j, i+1, j-1) + matrix_vx(i+1, j-1)) < best_score: best_score = score
