...
def matrix_yhx(i, j, k, l):
    ...
    # single-stranded
    if score := (Q_wave + matrix_yhx(i + 1, j, k, l)) < best_score: best_score = score
    if score := (Q_wave + matrix_yhx(i, j - 1, k, l)) < best_score: best_score = score
    ...
    if score := (P_wave + M_wave + matrix_whx(i, j, k - 1, l + 1)) < best_score: best_score = score
