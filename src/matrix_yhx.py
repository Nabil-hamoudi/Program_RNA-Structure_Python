...
def matrix_yhx(i, j, k, l):
    ...
    # dangles
    if score := (parameters["L_wave"] + parameters["R_wave"] + parameters["P_wave"] + matrix_vhx(i+1, j-1, k, l)) < best_score: best_score = score
    if score := (parameters["L_wave"] + parameters["P_wave"] + matrix_vhx(i+1, j, k, l)) < best_score: best_score = score
    if score := (parameters["R_wave"] + parameters["P_wave"] + matrix_vhx(i, j-1, k, l)) < best_score: best_score = score
    ...
    for r in range(i, k+1):
        ...
        if score := ((2 * parameters["P_wave"]) + parameters["C_wave"](r, i, r+1, j) + matrix_vx(i, r) + matrix_vhx(r+1, j, k, l)) < best_score: best_score = score

    for s in range(l, j+1):
        if score := (matrix_yhx(i, s, k, l) + matrix_wxi(s+1, j)) < best_score: best_score = score
        if score := ((2 * parameters["P_wave"]) + parameters["C_wave"](s, i, s+1, j) + matrix_vhx(i, s, k, l) + matrix_vx(s+1, j)) < best_score: best_score = score
