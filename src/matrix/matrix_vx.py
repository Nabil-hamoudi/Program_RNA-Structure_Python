...
def matrix_vx(i, j):
    ...
    for l in range(i,j+1):
        for k in range(i, l+1):
            if score := (parameters["EIS2"](i, j, k, l) + matrix_vx.matrix_vx(k, l)) < best_score: best_score = score

    # Nested Multiloop
    for k in range(i, j+1):
        if score := (parameters["Pi"] + parameters["M"] + matrix_wxi.matrix_wxi(i+1, k)
        ...
        if score := (2 * parameters["Pi"] + coaxial_stacking(j-1, k+1, j, i)
                     + parameters["M"] + matrix_vx.matrix_vx(k+1, j-1) + matrix_wxi.matrix_wxi(i+1, k)
                     ) < best_score: best_score = score
        ...
        for k in range(i, j_prime+1):
            for i_prime in range(i, k+1):
                if score := (3 * parameters["Pi"] + coaxial_stacking(k, i_prime, k+1, j_prime)
                             + parameters["M"] + matrix_vx.matrix_vx(i_prime, k) + matrix_vx.matrix_vx(k+1, j_prime)
                             ) < best_score: best_score = score
