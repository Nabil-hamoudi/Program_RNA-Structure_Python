...
def matrix_yhx(i, j, k, l, matrix, sequence):
    ...
    # dangles
    if score := (parameters["L_wave"](i, i+1, j-1) + parameters["R_wave"](j, i+1, j-1) + parameters["P_wave"] + matrix_vhx.matrix_vhx(i+1, j-1, k, l, matrix, sequence)) < best_score: best_score = score
    if score := (parameters["L_wave"](i, i+1, j) + parameters["P_wave"] + matrix_vhx.matrix_vhx(i+1, j, k, l, matrix, sequence)) < best_score: best_score = score
    if score := (parameters["R_wave"](j, i, j-1) + parameters["P_wave"] + matrix_vhx.matrix_vhx(i, j-1, k, l, matrix, sequence)) < best_score: best_score = score
    ...
    for r in range(i, k+1):
        ...
        if score := ((2 * parameters["P_wave"]) + parameters["C_wave"](r, i, r+1, j) + matrix_vx.matrix_vx(i, r, matrix, sequence) + matrix_vhx.matrix_vhx(r+1, j, k, l, matrix, sequence)) < best_score: best_score = score
    ...
    for s in range(l, j+1):
        ...
        if score := ((2 * parameters["P_wave"]) + parameters["C_wave"](s, i, s+1, j) + matrix_vhx.matrix_vhx(i, s, k, l, matrix, sequence) + matrix_vx.matrix_vx(s+1, j, matrix, sequence)) < best_score: best_score = score

    for r in range(i, k+1):
        for s in range(l, j+1):
            if score := (matrix_yhx(i, j, r, s, matrix, sequence) + EIS2(r, s, k, l)) < best_score: best_score = score
