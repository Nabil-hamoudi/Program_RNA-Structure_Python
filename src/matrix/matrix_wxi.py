...
def matrix_wxi(i, j):
    ...
    # Paired (9)
    if score := (parameters["Pi"] + matrix_vx.matrix_vx(i,j)) < best_score: best_score = score

    # Dangles
    if score := (dangle_Li(i, i+1, j-1) + dangle_Ri(j, i+1, j-1) + parameters["Pi"]
                 + matrix_vx.matrix_vx(i+1, j-1)) < best_score: best_score = score

    if score := (dangle_Li(i, i+1, j) + parameters["Pi"] + matrix_vx.matrix_vx(i+1, j)
                 ) < best_score: best_score = score

    if score := (dangle_Ri(j, i, j-1) + parameters["Pi"] + matrix_vx.matrix_vx(i, j-1)
                 ) < best_score: best_score = score
    ...
    for r in range(i, j+1):
        for l in range(i, r+1):
            for k in range(i, l+1):
                if score := (parameters["Gwi"] + matrix_whx.matrix_whx(i,r, k,l) + 
