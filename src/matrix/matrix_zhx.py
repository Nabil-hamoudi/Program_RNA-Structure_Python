...
def matrix_zhx(i, j, k, l, matrix, sequence):
    ...
    # dangles
    if (score := (parameters["L_wave"](l, k-1, l+1, sequence) + parameters["R_wave"](k, k-1, l+1, sequence) + parameters["P_wave"] + matrix_vhx.matrix_vhx(i, j, k-1, l+1, matrix, sequence))) < best_score: best_score = score
    if (score := (parameters["R_wave"](k, k-1, l, sequence) + parameters["P_wave"] + matrix_vhx.matrix_vhx(i, j, k-1, l, matrix, sequence))) < best_score: best_score = score
    if (score := (parameters["L_wave"](l, k, l+1, sequence) + parameters["P_wave"] + matrix_vhx.matrix_vhx(i, j, k, l+1, matrix, sequence))) < best_score: best_score = score
    ...
    for r in range(i, k+1):
        ...
        if (score := ((2 * parameters["P_wave"]) + parameters["C_wave"](r, l, r+1, k, sequence) + matrix_vhx.matrix_vhx(i, j, r, l, matrix, sequence) + matrix_vx.matrix_vx(r+1, k, matrix, sequence))) < best_score: best_score = score
    ...
    for s in range(l, j+1):
        ...
        if (score := ((2 * parameters["P_wave"]) + parameters["C_wave"](s-1, l, s, k, sequence) + matrix_vhx.matrix_vhx(i, j, k, s, matrix, sequence) + matrix_vx.matrix_vx(l, s-1, matrix, sequence))) < best_score: best_score = score

    for r in range(i, k+1):
        for s in range(l, j+1):
            if (score := (parameters["EIS2"](i, j, r, s, sequence) + matrix_zhx(r, s, k, l, matrix, sequence))) < best_score: best_score = score
