...
def matrix_whx(i, j, k, l):
    ...
    if (k+1) == l:
        whx[j][l][k][i] = matrix_wx(i, j)
        return matrix_wx(i, j)
    ...
    if score := parameters["L_wave"](i, i+1, j) + parameters["R_wave"](k, k-1, l) \
            + (2 * parameters["P_wave"]) + matrix_vhx(i+1, j, k-1, l) < best_score:
                best_score = score

    if score := parameters["L_wave"](l-1, k, l) + parameters["R_wave"](j, i, j-1) \
            + (2 * parameters["P_wave"]) + matrix_vhx(i, j-1, k, l+1) < best_score:
                best_score = score
    ...
    for r in range(i, k+1):
        if score := (matrix_wxi(i, r) + matrix_whx(r+1, j, k, l)) < best_score: best_score = score
        
        if score := 2 * parameters["P_wave"] + coaxial_stacking(r, i, r+1, j) \
                + matrix_vx(i, r) + matrix_zhx(r+1, j, k, l) < best_score: best_score = score

        if score := (matrix_wxi(r+1, k) + matrix_whx(i, j, r, l)) < best_score: best_score = score


    for s in range(l, j+1):
        if score := 2 * parameters["P_wave"] + coaxial_stacking(r, l, r+1, k) \
                + matrix_yhx(i, j, r, l) + matrix_vx(r+1, k) < best_score: best_score = score

        if score := (matrix_wxi(s+1, j) + matrix_whx(i, s, k, l)) < best_score: best_score = score

        if score := 2 * parameters["P_wave"] + coaxial_stacking(s, i, s+1, j) \
                + matrix_zhx(i, s, k, l) + matrix_vx(s+1, j) < best_score: best_score = score

        if score := (matrix_wxi(l, s) + matrix_whx(i, j, k, s+1)) < best_score: best_score = score
            ...
            for r_prime in range(i, k+1):
                for r in range(i, r_prime+1):
                    ...
                    if score := 2 * parameters["P_wave"] + parameters["Gwh"] \
                            ...
                            + matrix_zhx(i, s_prime, k, s) + matrix_yhx(l, j, s-1, s_prime + 1) \
                    ...
                    if score := 2 * parameters["P_wave"] + parameters["Gwh"] \
                            ...
                            + matrix_zhx(r, j, r_prime, l) + matrix_yhx(i, k, r-1, r_prime + 1) \
                            < best_score: best_score = score

                    if score := 2 * parameters["P_wave"] + parameters["Gwh"] \
                            + coaxial_stacking_wave(r_prime, l, r_prime + 1, r-1) \
                            + matrix_yhx(r, j, r_prime, l) + matrix_yhx(i, k, r-1, r_prime + 1) \
                            < best_score: best_score = score
